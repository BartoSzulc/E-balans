...
@if ($our_materialy && $our_materialy['add_material'])
    <section class="overflow-x-clip our-materialy-section mt-50 lg:mt-120">
        <div class="container mx-auto">
            <div class="lg:text-center mb-30 lg:mb-40 text-h2 text-color-3">
                {!! $our_materialy['title'] !!}
            </div>
            <div class="grid grid-cols-1 gap-16 materials-grid lg:grid-cols-2">
                ...
                @foreach ($our_materialy['add_material'] as $material)
                    <div
                        class="material-item min-h-360 bg-white shadow-special rounded-32 flex justify-between max-lg:gap-20 overflow-hidden {{ $loop->first ? 'col-span-full' : '' }}">
