        ...
        <div class="container mx-auto">
            <div class="grid grid-cols-12 gap-16 gap-y-24">
                <div class="col-span-full lg:col-span-5">
                    @if ($o_nas['images'])
                        <div class="relative images-gallery lg:h-540 w-469">
                            ...
                            @foreach ($o_nas['images'] as $index => $image_item)
                                @if ($image_item['image'])
                                    <div
                                        class="gallery-item shadow-special {{ $index === 0 ? 'h-280 lg:h-420 relative' : '-z-1 lg:absolute h-280 bottom-0 -right-123' }}">
                ...
                <div class="col-span-full lg:col-span-6 lg:col-start-7">
                    <div class="flex flex-col gap-24 o-nas-content">
                        @if ($o_nas['title'])
                            <div class="section-title text-h2 text-color-3">
                                {!! $o_nas['title'] !!}
                            </div>
                        @endif
                        ...
                        @if ($o_nas['add_services'])
                            <div class="flex flex-wrap justify-between lg:mt-16 services-grid gap-y-16">
                                @foreach ($o_nas['add_services'] as $service)
                                    <div
                                        class="flex items-center gap-16 py-8 pl-6 pr-32 bg-white rounded-full min-w-320 service-item shadow-special">
                                        @if ($service['image'])
                                            <div
