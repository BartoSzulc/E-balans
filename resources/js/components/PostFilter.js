...
class PostFilter {
  constructor() {
    // DOM elements
    this.$categoryButtons = document.querySelectorAll('.category-filter-btn');
    this.$categoryButtonsContainer = document.getElementById('category-filter-container');
    this.$postGrid = document.getElementById('post-grid');
    ...
    // AJAX data
    this.ajaxUrl = window.post_filter_ajax?.ajax_url || '';
    this.nonce = window.post_filter_ajax?.nonce || '';
    
    // Only initialize if elements exist and AJAX URL is available
    if (this.$categoryButtonsContainer && this.$postGrid && this.ajaxUrl) {
      this.init();
    } else {
      //console.warn('Post filter: Missing required elements or AJAX URL');
    }
  }
  ...
  init() {
    ...
    // Find active category button and set initial state
    const activeButton = document.querySelector('.category-filter-btn.active');
    if (activeButton) {
      this.currentCategory = activeButton.dataset.category;
    } else if (this.$categoryButtons.length > 0) {
      // Set first category as default if none is active
      const firstButton = this.$categoryButtons[0];
      this.currentCategory = firstButton.dataset.category;
  ...
  setupMobileScroll() {
    if (this.$categoryButtonsContainer) {
      // Add horizontal scrolling behavior for mobile
      this.$categoryButtonsContainer.classList.add('overflow-x-auto', 'scrollbar-hide');
  ...
  bindEvents() {
    ...
    if (this.$categoryButtons) {
      this.$categoryButtons.forEach(button => {
        button.addEventListener('click', (e) => {
          ...
          if (this.currentCategory === button.dataset.category) return;
          ...
          // Update URL without page parameter when changing category
          this.updateUrlWithoutRefresh(this.currentCategory, null);
    ...
    document.addEventListener('click', (e) => {
      ...
      if (pageLink && !pageLink.closest('.active')) {
        ...
        if (page !== this.currentPage) {
          ...
          this.updateUrlWithoutRefresh(this.currentCategory, page);
    ...
    window.addEventListener('popstate', (e) => {
      ...
      if (category !== this.currentCategory || page !== this.currentPage) {
        // Update state variables
        this.currentCategory = category;
        this.currentPage = page;
        
        // Update category buttons UI using the active button setter
  ...
  updateUrlWithoutRefresh(category, page) {
    const url = new URL(window.location);
    
    // Update category in URL if it exists and isn't 'all'
  ...
  showLoadingState(showIndicator = true) {
    ...
    // Hide posts during loading
    if (this.$postGrid) {
      this.$postGrid.style.opacity = '0.5';
      this.$postGrid.style.pointerEvents = 'none';
    }
  ...
  hideLoadingState() {
    ...
    // Show posts after loading
    if (this.$postGrid) {
      this.$postGrid.style.opacity = '1';
      this.$postGrid.style.pointerEvents = 'auto';
    }
  ...
  loadPosts() {
    ...
    formData.append('posts_per_page', 15) // Show 15 posts per page
    ...
    fetch(this.ajaxUrl, {
      ...
      .then(response => {
        ...
        if (this.$postGrid) {
          this.$postGrid.innerHTML = response.html;
        }
  ...
  handleError() {
    ...
    // Restore post grid visual state
    if (this.$postGrid) {
      this.$postGrid.style.opacity = '1';
      this.$postGrid.style.pointerEvents = 'auto';
      this.$postGrid.innerHTML = `
        <div class="col-span-full text-center text-color-1">
          <p>Wystąpił błąd podczas ładowania postów. Spróbuj ponownie.</p>
        </div>
      `;
    }
