...
export function initGallerySlider() {
  ...
  document.querySelectorAll('.swiperGallery').forEach((el) => {
    const galleryId = el.dataset.gallery;
    
    // Find navigation buttons for this specific gallery
    const nextButton = document.querySelector(`.swiperGallery__nav--next[data-gallery="${galleryId}"]`);
    const prevButton = document.querySelector(`.swiperGallery__nav--prev[data-gallery="${galleryId}"]`);
    ...
    let GallerySwiper = new Swiper(el, {
      ...
      breakpoints: {
        1024: {
          slidesPerView: 3,
          spaceBetween: responsivePx(97), // 20 here is your base value that will be scaled
        }
      },
      on: {
        init: function() {
